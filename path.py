...
#obstacle1 = np.array([0, 0, 0.04, 0.2])
obstacle_list = []
def check_point_ok(new_coords):
    if (new_coords[0] < -2 or new_coords[1] < -2 or new_coords[2] < -2 or new_coords[0] > 2 or new_coords[1] > 2 or new_coords[2] > 2):
        return False
...
start_coords = np.array([0., -0.15, 0.4])
finish_coords = np.array([0., 0.2, 0.1])
...
while finish_point is None:
    if all_points[0].level % 4 == 0:
        all_points.sort(key=lambda x: (x.coordinates[0]-finish_coords[0])**2 + (x.coordinates[1]-finish_coords[1])**2 + (x.coordinates[2]-finish_coords[2])**2)
...
for i in range(len(path_points)-1):
    ax.plot([path_points[i][0],path_points[i+1][0]],[path_points[i][1],path_points[i+1][1]],[path_points[i][2],path_points[i+1][2]])
